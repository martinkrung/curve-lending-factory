...
init_params: public(uint256, uint256, uint256, uint256)


@external
def __init__(factory: Factory, amm: IAMM, borrowed_token: ERC20, init_params: uint256, uint256, uint256, uint256):
    """
    ...
    """
    assert init_params[0] >= MIN_UTIL
    assert init_params[0] <= MAX_UTIL
    assert init_params[1] >= MIN_LOW_RATIO
    assert init_params[2] <= MAX_HIGH_RATIO
    assert init_params[1] < high_ratio
    assert init_params[3] <= MAX_RATE_SHIFT

    FACTORY = factory
    AMM = amm
    BORROWED_TOKEN = borrowed_token
    p: Parameters = self.get_params(init_params[0], init_params[1], init_params[2], init_params[3])
    self.parameters = p
